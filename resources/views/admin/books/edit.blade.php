    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form id="editBookForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" id="bookId" name="id" value="{{ $id ?? '' }}">
                ...
                <div class="mb-3">
                    ...
                    <small class="text-secondary">Leave blank to keep current cover</small>
...
async function loadBookData() {
    try {
        ...
        document.getElementById('is_free').value = book.is_free ? '1' : '0';
        document.getElementById('description').value = book.description || '';

        // Show current cover
        if (book.cover_image) {
            document.getElementById('currentCover').innerHTML = `
                <img src="/storage/${book.cover_image}" alt="Current Cover" style="max-height: 200px; border-radius: 8px;">
            `;
        }

        // Disable price if free
        if (book.is_free) {
...
document.getElementById('editBookForm').addEventListener('submit', async (e) => {
    ...
    // Validate
    if (!data.title || !data.isbn || !data.author || !data.genre) {
    ...
    try {
        ...
        if (response.success || response.data) {
            ...
            if (coverFile) {
                try {
                    ...
                    const uploadResponse = await fetch(`${API_URL}/books/${bookId}/upload-cover`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (!uploadResponse.ok) {
                        ...
                        // Continue anyway - book was updated successfully
                    }
                } catch (uploadError) {
            ...
            showAlert(response.message || 'Failed to update book', 'error');
        ...
        showAlert('Failed to update book. Please try again.', 'error');
        submitBtn.disabled = false;
        btnText.classList.remove('d-none');
        spinner.classList.add('d-none');
    }
